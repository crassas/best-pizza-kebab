...
export function OrderDrawer() {
  ...
  const send = () => {
    ...
    const lines = items.map(
      (i) =>
        `• ${i.quantity}x ${i.name}${i.sizeName ? ` (${i.sizeName})` : ""} — ${(i.price * i.quantity).toFixed(2)}€`,
    );
    const intro = isPt
    ...
    <div className="fixed inset-0 z-70 flex justify-end bg-black/70" onClick={() => setOpen(false)}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {!items.length ? (
            ...
            items.map((i) => (
              <div
                key={`${i.id}-${i.sizeName || ""}`}
                className="rounded-xl border-2 border-black bg-surface-card p-3 shadow-fastfood"
              >
                <div className="flex justify-between gap-2">
                  <div>
                    <strong className="text-white">{i.name}</strong>
                    {i.sizeName && <div className="text-xs text-muted">{i.sizeName}</div>}
                    <div className="font-bold text-brand-yellow">{i.price.toFixed(2)}€</div>
                  </div>
                  <button onClick={() => removeItem(i.id, i.sizeName)} aria-label="Remove">
                    <Trash2 className="size-4 text-brand-red" />
                  </button>
                </div>
                <div className="mt-3 flex items-center gap-2">
                  <button
                    onClick={() => updateQuantity(i.id, i.quantity - 1, i.sizeName)}
        ...
        <div className="border-t-2 border-line p-4 space-y-3">
          ...
          <div className="rounded-lg border-2 border-line bg-surface-card p-3">
            <div className="flex items-start gap-2">
              <WhatsAppIcon size="sm" className="mt-0.5 shrink-0 text-bolt" />
              <p className="text-xs leading-relaxed text-cream/80">
                {isPt
                  ? "O site vai criar automaticamente a mensagem com os produtos, quantidades e total. O WhatsApp abre já preenchido; só tens de confirmar o envio."
                  : "The site automatically creates the message with the items, quantities and total. WhatsApp opens already filled in; you only confirm the send action."}
              </p>
            </div>
          </div>

          <button
            disabled={!items.length || settings.isOnlineOrderingPaused}
            onClick={send}
            className="flex w-full items-center justify-center gap-2 rounded-xl border-3 border-black bg-bolt py-3 font-display text-xl font-black uppercase text-black shadow-fastfood disabled:opacity-40"
          >
          ...
          <p className="text-[11px] leading-relaxed text-muted">
            {isPt
              ? "O envio da mensagem não confirma automaticamente o pedido. O restaurante confirma antes da preparação."
              : "Sending the message does not automatically confirm the order. The restaurant confirms it before preparation."}
          </p>
