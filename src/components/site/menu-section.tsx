...
import { getCategoryAnchor, menu, type MenuCategory, type MenuItem } from "@/lib/restaurant";
    ...
    <section id="menu" className="relative scroll-mt-[calc(4rem+env(safe-area-inset-top))] border-t-4 border-black bg-brand-black text-cream">
      <div className="mx-auto max-w-7xl px-4 pt-12 sm:px-6 sm:pt-16">
        <div className="flex flex-col items-start md:flex-row md:items-end md:justify-between gap-4">
          <div><div className="inline-block rounded-xs bg-brand-red px-2.5 py-1 text-xs font-black uppercase tracking-widest text-white mb-2 shadow-xs">★ {isPt ? "CARDÁPIO OFICIAL" : "OFFICIAL MENU"} ★</div><h2 className="font-display text-4xl sm:text-6xl uppercase tracking-tight text-white leading-none">{isPt ? "O Nosso Menu Completo" : "Our Full Menu"}</h2><p className="mt-2 text-sm sm:text-base text-muted max-w-xl">{isPt ? "Kebabs, durum, pizzas, hambúrgueres e falafel. Selecione os seus itens favoritos e adicione ao seu pedido!" : "Kebabs, durum, pizzas, burgers and falafel. Choose your favorites and build your order tray!"}</p></div>
