...
## Agent Reach Integration Rule
When a task requires public web/social/platform research that Agent Reach supports, inspect the Agent Reach skill and select the appropriate verified backend. Prefer existing native Google AI Studio tools when they provide equivalent or better capability. Never claim freshness or platform access without execution evidence.
